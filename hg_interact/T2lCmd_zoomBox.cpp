...
Box2F Cmd_zoomBox::newDisplay_( const Point2F& pt, Display& view )
{
    ...
    return Rectangle2::boxFrom2pointsAndRatio( point_, pt, fabs(viewSize.w()/viewSize.h()) );*/
    ...
    if ( ratioNeeded > ratio ) {
        ...
        DY = ( ( dy > 0.0 ) ? 1.0 : -1.0 ) * fabs(dx/ratioNeeded);
    }
...
void Cmd_zoomBox::enterPoint( const Point2F& pt, Display& view )
{
    ...
    if ( pointKnown_ )
    {
        ...
        //view.scaleSet( view.getRefCanvas().scale()*view.size().w()/newDisplay.x().getLength(), Scale2::X );
        view.scaleSet( view.getRefCanvas().scale()*view.size().h()/newDisplay.y().getLength() );
...
//===================================================================
QString Cmd_zoomBox::hint(void) const
{
    if (pointKnown_) {
        return "enter second point of box";
    }

    return "enter first point of box";
}
