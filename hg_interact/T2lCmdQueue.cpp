...
//=============================================================================
void CmdQueue::enterPointStright( const Point3F& pt, Display& view )
{
    activeCommand()->enterPoint(Point2F(pt.x(), pt.y()), view);
    activeCommand()->enterPoint(pt, view);
    lastPoint_ = pt;
}

//=============================================================================
void CmdQueue::enterPoint( const Point3F& pt, Display& view )
{
    originalPoint_ = pt;
    ...
    if (tentative_->entered_ == true) {
        ...
        Point2F ptg = gridCalc(Point2F(PT.x(), PT.y()));
        PT = Point3F(ptg.x(), ptg.y(), 0);
    }

    stringstream cmd;
    cmd << "xy \"" << PT.x() << "\" \"" << PT.y() << "\" \"" << PT.z() << "\""; //APTA
