...
//=============================================================================
void Display::scaleSet(double scale)
{
    scale_ = scale;
...
bool Display::isBitmapFresh_()
{
    ...
    else if ( scale() > (bitmap_->bmpScale_ + 10e-5) ) result = 3;
    else if ( scale() < (bitmap_->bmpScale_ - 10e-5) ) result = 4;
...
{
    Canvas result(scale_, 1.0);
    //result.scaleSet(scale_.scaleY());

//    Box2F bound;
//    bound.inflateTo(origin());
//    bound.inflateTo(Point2F(origin().x()+size().w(), origin().y()+size().h()));
//    result.boundSet(bound); //APTA💊
...
//=============================================================================
QPixmap* Display::bitmap()
{
    return bitmap_->bmpPixmap;
}
