...
void Widget::paintEvent( QPaintEvent* )
{
    ...
    double dx = (double)(bitmap_->bmpOrigin_.x() - origin().x() ) * scale_ * ppm_;
    double dy = (double)(bitmap_->bmpOrigin_.y() - origin().y() ) * scale_ * ppm_ + ( bitmap_->bmpPixmap->height()-height() );
...
{
    return Size2F( fabs( width()-edge_[EDGE_LEFT]-edge_[EDGE_RIGHT]) / (ppm_*fabs(scale())),
                   fabs( height()-edge_[EDGE_TOP]-edge_[EDGE_BOTTOM]) / (ppm_*fabs(scale())) );
}
...
{
    double x = pt.x()*1.0/(((double)ppm_)*scale()) + origin().x();
    double y = (((double)height())-pt.y())*1.0/(((double)ppm_)*scale()) + origin().y();
    return Point2F(x, y);
}
...
void Widget::cleanDrawing_(QPainter& paint)
{
    ...
    int x = (int)((double)(bitmap_->bmpOrigin_.x() - origin().x() ) * scale_ * ppm_);
    int y = (int)((double)(bitmap_->bmpOrigin_.y() - origin().y() ) * scale_ * ppm_);
